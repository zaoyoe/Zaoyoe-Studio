...
func normalizeOptionValue(key string, value string) (string, error) {
	...
	case "blocked_country_codes":
		...
		for _, code := range codes {
			...
			if len(code) != 2 ||
				...
				return "", fmt.Errorf("%s contains invalid country code %q", key, code)
			}
	...
	case "unknown_region_policy":
		normalized := strings.ToLower(strings.TrimSpace(value))
		if normalized != "allow" && normalized != "deny" {
			return "", fmt.Errorf("%s must be allow or deny", key)
		}
		return normalized, nil
	case "confirmation_frequency":
		normalized := strings.ToLower(strings.TrimSpace(value))
		if normalized != "once_per_revision" && normalized != "always" && normalized != "interval" {
			return "", fmt.Errorf("%s must be once_per_revision, always, or interval", key)
		}
		return normalized, nil
	case "confirmation_revision":
		normalized := strings.TrimSpace(value)
		if normalized == "" || !utf8.ValidString(normalized) || utf8.RuneCountInString(normalized) > 64 {
			return "", fmt.Errorf("%s must be non-empty and at most 64 characters", key)
		}
		return normalized, nil
	case "confirmation_interval_hours":
		normalized := strings.TrimSpace(value)
		hours, err := strconv.Atoi(normalized)
		if err != nil || hours < 1 || hours > 8760 {
			return "", fmt.Errorf("%s must be an integer between 1 and 8760", key)
		}
...
func handleConfigUpdate(key, value string) bool {
	...
	configName := parts[0]
	configKey := parts[1]
	if configName == system_setting.RegionalRestrictionConfigName {
		_ = system_setting.UpdateRegionalRestrictionSettings(map[string]string{configKey: value})
		return true
	}
