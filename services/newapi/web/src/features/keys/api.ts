...
export async function getRegionalRestrictionStatus(
  scope: RegionalRestrictionScope
): Promise<RegionalRestrictionStatus> {
  const res = await api.get<ApiResponse<RegionalRestrictionStatus>>(
    '/api/token/regional-restriction',
    {
      params: { scope },
  ...
  if (!res.data.success || !res.data.data) {
    throw new Error(res.data.message || 'Regional restriction check failed')
  }
