...
import { TypeLedHome } from './type-led-home'
...
export function Home() {
  ...
  const homeHeaderProps = { homeUrl: mainSiteUrl }
  ...
  if (!isLoaded) {
    return (
      <main className='flex min-h-screen items-center justify-center bg-[#050505]'>
        <div className='text-white/55'>{t('Loading...')}</div>
      </main>
    )
  }
  ...
  if (content) {
    if (isUrl) {
      return (
        <PublicLayout showMainContainer={false} headerProps={homeHeaderProps}>
          {/*
    ...
    return (
      <PublicLayout headerProps={homeHeaderProps}>
  ...
  return <TypeLedHome isAuthenticated={isAuthenticated} />
}
