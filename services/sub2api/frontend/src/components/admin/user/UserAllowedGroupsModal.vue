<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-4 sm:space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-start gap-3 rounded-xl bg-gradient-to-r from-primary-50 to-primary-100 p-3 dark:from-primary-900/30 dark:to-primary-800/20 sm:items-center sm:gap-4 sm:rounded-2xl sm:p-5">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white shadow-sm dark:bg-dark-700 sm:h-14 sm:w-14">
          <span class="text-lg font-semibold text-primary-600 dark:text-primary-400 sm:text-2xl">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="min-w-0 flex-1">
          <p class="truncate font-semibold text-gray-900 dark:text-white sm:text-lg">{{ user.email }}</p>
          <p class="mt-1 text-sm leading-5 text-gray-600 dark:text-gray-400">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="loading" class="flex justify-center py-8 sm:py-12">
        <svg class="h-8 w-8 animate-spin text-primary-500 sm:h-10 sm:w-10" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-4 sm:space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          ...
          <div class="grid gap-2 sm:gap-3">
            <div
              ...
              :key="config.groupId"
              class="group relative overflow-hidden rounded-xl border-2 p-3 transition-all duration-200 sm:p-4"
              ...
              <div class="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    <span class="min-w-0 truncate font-semibold text-gray-900 dark:text-white sm:text-base">{{ config.groupName }}</span>
                  ...
                  <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm sm:mt-1.5 sm:gap-3">
                ...
                <!-- 专属倍率输入 -->
                <div class="flex w-full flex-shrink-0 items-center gap-3 sm:w-auto">
                  <label class="shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner min-w-0 flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-primary-500 sm:w-24 sm:flex-none"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          ...
          <div class="grid gap-2 sm:gap-3">
            <div
              ...
              :key="config.groupId"
              class="relative overflow-hidden rounded-xl border-2 border-green-200 bg-green-50/50 p-3 dark:border-green-800/50 dark:bg-green-900/10 sm:p-4"
            >
              <div class="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
        ...
        <!-- 无分组提示 -->
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-8 text-center sm:py-12">
          <div class="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-gray-100 dark:bg-dark-700 sm:mb-4 sm:h-16 sm:w-16">
            <svg class="h-7 w-7 text-gray-400 sm:h-8 sm:w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
