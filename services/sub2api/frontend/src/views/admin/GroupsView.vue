...
const applyRouteFilters = () => {
  const platform = normalizeRouteQueryValue(route.query.platform);
  const status = normalizeRouteQueryValue(route.query.status);
  const isExclusive = normalizeRouteQueryValue(route.query.is_exclusive);
  const search = normalizeRouteQueryValue(route.query.search);

  filters.platform = platformFilterOptions.value.some((option) => option.value === platform)
    ? platform
    : "";
  filters.status = statusOptions.value.some((option) => option.value === status)
    ? status
    : "";
  filters.is_exclusive = exclusiveOptions.value.some((option) => option.value === isExclusive)
    ? isExclusive
    : "";
