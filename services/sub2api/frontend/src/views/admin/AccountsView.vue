...
const handleDataImported = (payload?: { targetGroupId?: number; targetGroupName?: string; targetGroupPlatform?: 'gemini' }) => {
  showImportData.value = false

  if (payload?.targetGroupPlatform === 'gemini' && typeof payload.targetGroupId === 'number') {
    const query: Record<string, string> = {
      platform: 'gemini',
      focus_group_id: String(payload.targetGroupId)
    }
    const search = payload.targetGroupName?.trim()
    if (search) {
      query.search = search
    }
    void router.push({ name: 'AdminGroups', query }).catch((error) => {
      console.error('Failed to navigate to imported Gemini group:', error)
      reload()
    })
    return
  }
