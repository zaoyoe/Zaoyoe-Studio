export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportLocalExportTitle: 'Local export command',
      dataImportLocalExportHint:
        'If you forget how to generate the import JSON, run these commands in your local terminal first.',
      dataImportLocalExportCopy: 'Copy command',
      dataImportLocalExportCopied: 'Local export command copied to clipboard',
      dataImportLocalExportSourceAntigravity: 'Antigravity',
      dataImportLocalExportSourceClaude: 'Claude Code OAuth',
      dataImportLocalExportSourceGemini: 'Gemini CLI OAuth',
      dataImportLocalExportSourceAntigravityNote:
        'The Antigravity exporter reads ~/.cli-proxy-api/antigravity-*.json local cache files by default and enables mixed_scheduling by default.',
      dataImportLocalExportSourceClaudeNote:
        'The Claude exporter reads local OAuth credentials from Keychain or .credentials.json, not the browser sessionKey cookie. For multiple accounts, repeat --config-dir or use --config-dir-glob.',
      dataImportLocalExportSourceGeminiNote:
        'The Gemini exporter reads local Gemini CLI OAuth credentials from macOS Keychain or ~/.gemini/gemini-credentials.json. If the CLI is currently in API key mode, no OAuth accounts will be exported.',
      dataImportLocalExportNote:
        'Run this in your local terminal. If you are already inside the sub2api repo, you can skip the first cd line.',
      dataImportLocalSyncTitleAntigravity: 'Antigravity direct sync command',
      dataImportLocalSyncHintAntigravity:
        'If you already have admin credentials, you can sync local Antigravity cache accounts directly to the backend without manually uploading JSON.',
      dataImportLocalSyncTitleClaude: 'Claude direct sync command',
      dataImportLocalSyncHintClaude:
        'If you already have admin credentials, you can sync local Claude OAuth accounts directly to the backend without manually uploading JSON.',
      dataImportLocalSyncTitleGemini: 'Gemini direct sync command',
      dataImportLocalSyncHintGemini:
        'If you already have admin credentials, you can sync local Gemini CLI OAuth accounts directly to the backend without manually uploading JSON.',
      dataImportLocalSyncBaseUrlLabel: 'Backend URL',
      dataImportLocalSyncBaseUrlPlaceholder: 'https://your-sub2api.example.com',
      dataImportLocalSyncAuthModeLabel: 'Auth mode',
      dataImportLocalSyncAuthModeApiKey: 'Admin API Key',
      dataImportLocalSyncAuthModeToken: 'Admin JWT',
      dataImportLocalSyncCopy: 'Copy sync command',
      dataImportLocalSyncCopied: 'Direct sync command copied to clipboard',
      dataImportLocalSyncCredentialLabelApiKey: 'Admin API Key',
      dataImportLocalSyncCredentialLabelToken: 'Admin JWT',
      dataImportLocalSyncCredentialPlaceholderApiKey: 'YOUR_ADMIN_API_KEY',
      dataImportLocalSyncCredentialPlaceholderToken: 'YOUR_ADMIN_JWT',
      dataImportLocalSyncNoteAntigravity:
        'This example uses an Admin API Key by default. If you prefer an admin JWT, replace --admin-api-key with --admin-token. Default group binding stays skipped, and mixed_scheduling is enabled by default.',
      dataImportLocalSyncNoteClaude:
        'This example uses an Admin API Key by default. If you prefer an admin JWT, replace --admin-api-key with --admin-token. Default group binding stays skipped.',
      dataImportLocalSyncNoteGemini:
        'This example uses an Admin API Key by default. If you prefer an admin JWT, replace --admin-api-key with --admin-token. Default group binding stays skipped; add project_id or tier_id flags if you need to force a specific Gemini account flavor.',
      dataImportDesktopHelperTitle: 'Desktop helper one-click scan',
      dataImportDesktopHelperHint:
        'If the desktop helper is running on this machine, the admin page can scan local CLI login state directly without manually choosing JSON files.',
      dataImportDesktopHelperCopyStartCommand: 'Copy start command',
      dataImportDesktopHelperStartCommandCopied: 'Desktop helper start command copied to clipboard',
      dataImportDesktopHelperStartCommandNote:
        'Start the desktop helper once in your local terminal, then this page can read CLI login state directly through 127.0.0.1.',
      dataImportDesktopHelperDownload: 'Scan and download JSON',
      dataImportDesktopHelperImport: 'Scan and import directly',
      dataImportDesktopHelperProcessing: 'Scanning...',
      dataImportDesktopHelperUnavailable:
        'Desktop helper is not reachable. Start it locally with python3 tools/desktop_cli_bridge.py first.',
      dataImportDesktopHelperInvalidResponse: 'Desktop helper returned an invalid response',
      dataImportDesktopHelperReady: 'Desktop helper found {count} importable account(s)',
      dataImportDesktopHelperSummary:
        'Desktop helper scan finished: exported {exported} account(s), skipped {skipped} local record(s)',
      dataImportDesktopHelperSourceAntigravityNote:
        'This will scan ~/.cli-proxy-api/antigravity-*.json and reuse the Gemini mixed scheduling and target group options below.',
      dataImportDesktopHelperSourceClaudeNote:
        'This will scan local Claude Code OAuth credentials from Keychain or .credentials.json and build importable account JSON directly.',
      dataImportDesktopHelperSourceGeminiNote:
        'This will scan local Gemini CLI OAuth credentials from Keychain or ~/.gemini/gemini-credentials.json and reuse the Gemini target group option below. If the CLI is currently using API key auth, no exportable OAuth account will be found.',
      dataImportLocalCacheTitle: 'Generate directly from local cache',
      dataImportLocalCacheHint:
        'You can select local antigravity cache JSON files here and build importable data in the browser without opening a terminal first.',
      dataImportLocalCacheMixedScheduling: 'Enable Gemini mixed scheduling after import',
      dataImportLocalCacheMixedSchedulingHint:
        'When enabled, imported Antigravity accounts are created with mixed_scheduling=true so they can join Gemini group scheduling immediately.',
      dataImportLocalCacheGeminiGroupLabel: 'Bind to Gemini group after import (optional)',
      dataImportLocalCacheGeminiGroupNone: 'Do not bind an extra group',
      dataImportLocalCacheGeminiGroupHint:
        'If you choose a Gemini group here, imported accounts will be created with that group_id already attached.',
      dataImportLocalCacheGeminiGroupSelected:
        'Imported accounts will be bound to the Gemini group: {group}',
      dataImportLocalCachePickFiles: 'Choose cache files',
      dataImportLocalCacheSelectFile: 'Choose one or more antigravity-*.json files',
      dataImportLocalCacheSelected: '{count} cache file(s) selected',
      dataImportLocalCacheDownload: 'Download import JSON',
      dataImportLocalCacheImport: 'Import cache directly',
      dataImportLocalCacheProcessing: 'Processing...',
      dataImportLocalCacheNote:
        'Files with disabled=true, missing refresh_token, or invalid JSON are skipped by default. Default group binding is still skipped during import.',
      dataImportLocalCacheNoFiles: 'Please choose local cache files first',
      dataImportLocalCacheNoAccounts: 'No importable local cache accounts were found',
      dataImportLocalCacheReady: '{count} importable account(s) generated',
      dataImportLocalCacheSummary:
        'Processed local cache files: exported {exported} account(s), skipped {skipped} file(s)',
