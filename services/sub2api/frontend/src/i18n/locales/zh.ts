export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportLocalExportTitle: '本地导出命令',
      dataImportLocalExportHint: '如果忘了如何生成导入 JSON，可以先在本机终端执行下面这组命令。',
      dataImportLocalExportCopy: '复制命令',
      dataImportLocalExportCopied: '本地导出命令已复制到剪贴板',
      ...
      dataImportLocalExportSourceAntigravityNote:
        'Antigravity 导出脚本默认读取 ~/.cli-proxy-api/antigravity-*.json 本地缓存，并默认启用 mixed_scheduling。',
      dataImportLocalExportSourceClaudeNote:
        'Claude 导出读取本地 OAuth 凭据（Keychain 或 .credentials.json），不是浏览器 sessionKey；多账号时可重复使用 --config-dir 或 --config-dir-glob。',
      dataImportLocalExportSourceGeminiNote:
        'Gemini 导出读取 Gemini CLI 本地 OAuth 凭据（macOS Keychain 或 ~/.gemini/gemini-credentials.json）。如果当前 CLI 处于 API Key 模式，则不会导出出任何 OAuth 账号。',
      dataImportLocalExportNote:
        '请在本地终端执行；如果已经位于 sub2api 仓库目录，可以省略第一行 cd。',
      dataImportLocalSyncTitleAntigravity: 'Antigravity 一键同步命令',
      dataImportLocalSyncHintAntigravity:
        '如果已经准备好管理员凭据，可以直接把本地 Antigravity 缓存账号同步到后台，无需再手动上传 JSON。',
      dataImportLocalSyncTitleClaude: 'Claude 一键同步命令',
      dataImportLocalSyncHintClaude:
        '如果已经准备好管理员凭据，可以直接把本地 Claude OAuth 账号同步到后台，无需再手动上传 JSON。',
      dataImportLocalSyncTitleGemini: 'Gemini 一键同步命令',
      dataImportLocalSyncHintGemini:
        '如果已经准备好管理员凭据，可以直接把本地 Gemini CLI OAuth 账号同步到后台，无需再手动上传 JSON。',
      dataImportLocalSyncBaseUrlLabel: '后台地址',
      dataImportLocalSyncBaseUrlPlaceholder: 'https://your-sub2api.example.com',
      dataImportLocalSyncAuthModeLabel: '认证方式',
      dataImportLocalSyncAuthModeApiKey: 'Admin API Key',
      dataImportLocalSyncAuthModeToken: '管理员 JWT',
      dataImportLocalSyncCopy: '复制同步命令',
      dataImportLocalSyncCopied: '一键同步命令已复制到剪贴板',
      dataImportLocalSyncCredentialLabelApiKey: 'Admin API Key',
      dataImportLocalSyncCredentialLabelToken: '管理员 JWT',
      ...
      dataImportLocalSyncNoteAntigravity:
        '默认使用 Admin API Key；如果您更习惯管理员 JWT，也可以把 --admin-api-key 改成 --admin-token。脚本默认跳过后台默认分组绑定，并默认启用 mixed_scheduling。',
      dataImportLocalSyncNoteClaude:
        '默认使用 Admin API Key；如果您更习惯管理员 JWT，也可以把 --admin-api-key 改成 --admin-token。脚本默认跳过后台默认分组绑定。',
      dataImportLocalSyncNoteGemini:
        '默认使用 Admin API Key；如果您更习惯管理员 JWT，也可以把 --admin-api-key 改成 --admin-token。脚本默认跳过后台默认分组绑定；如需指定 project_id 或 tier_id，可继续追加对应参数。',
      dataImportDesktopHelperTitle: '桌面助手一键扫描',
      dataImportDesktopHelperHint:
        '如果您在本机运行桌面助手，后台可以直接扫描当前机器上的 CLI 登录态，无需手动挑选 JSON 文件。',
      dataImportDesktopHelperCopyStartCommand: '复制启动命令',
      dataImportDesktopHelperStartCommandCopied: '桌面助手启动命令已复制到剪贴板',
      dataImportDesktopHelperStartCommandNote:
        '先在本机终端启动一次桌面助手，之后这个页面就可以直接通过 127.0.0.1 自动读取 CLI 登录态。',
      dataImportDesktopHelperDownload: '扫描并下载 JSON',
      dataImportDesktopHelperImport: '扫描并直接导入',
      dataImportDesktopHelperProcessing: '扫描中...',
      dataImportDesktopHelperUnavailable:
        '未连接到本机桌面助手，请先在本机运行 python3 tools/desktop_cli_bridge.py。',
      dataImportDesktopHelperInvalidResponse: '桌面助手返回的数据格式无效',
      dataImportDesktopHelperReady: '桌面助手已扫描出 {count} 个可导入账号',
      dataImportDesktopHelperSummary:
        '桌面助手扫描完成：导出 {exported} 个账号，跳过 {skipped} 个本地配置',
      dataImportDesktopHelperSourceAntigravityNote:
        '当前会扫描 ~/.cli-proxy-api/antigravity-*.json，并沿用下面的 Gemini 混合调度与目标分组设置。',
      dataImportDesktopHelperSourceClaudeNote:
        '当前会扫描 Claude Code 本地 OAuth 凭据（Keychain 或 .credentials.json），并直接生成可导入的账号 JSON。',
      dataImportDesktopHelperSourceGeminiNote:
        '当前会扫描 Gemini CLI 本地 OAuth 凭据（Keychain 或 ~/.gemini/gemini-credentials.json），并沿用下面的 Gemini 目标分组设置；如果当前 CLI 是 API Key 模式，则不会找到可导出的 OAuth 账号。',
      dataImportLocalCacheTitle: '从本地缓存直接生成',
      dataImportLocalCacheHint:
        '可以直接选择本机的 antigravity 缓存 JSON 文件，在浏览器里生成可导入数据，无需先打开终端。',
      dataImportLocalCacheMixedScheduling: '导入后启用 Gemini 混合调度',
      dataImportLocalCacheMixedSchedulingHint:
        '开启后会为导入的 Antigravity 账号写入 mixed_scheduling=true，后续可直接参与 Gemini 分组调度。',
      dataImportLocalCacheGeminiGroupLabel: '导入后绑定 Gemini 分组（可选）',
      dataImportLocalCacheGeminiGroupNone: '不额外绑定分组',
      dataImportLocalCacheGeminiGroupHint:
        '如果这里选择了 Gemini 分组，导入出的账号会直接带上对应 group_ids。',
      dataImportLocalCacheGeminiGroupSelected: '当前将导入账号直接绑定到 Gemini 分组：{group}',
      dataImportLocalCachePickFiles: '选择缓存文件',
      dataImportLocalCacheSelectFile: '请选择一个或多个 antigravity-*.json 文件',
      dataImportLocalCacheSelected: '已选择 {count} 个缓存文件',
      dataImportLocalCacheDownload: '下载导入 JSON',
      dataImportLocalCacheImport: '直接导入缓存',
      dataImportLocalCacheProcessing: '处理中...',
      dataImportLocalCacheNote:
        '默认跳过 disabled=true、缺少 refresh_token 或格式无效的文件；导入时仍会跳过默认分组绑定。',
      dataImportLocalCacheNoFiles: '请先选择本地缓存文件',
      dataImportLocalCacheNoAccounts: '没有可导入的本地缓存账号',
      dataImportLocalCacheReady: '已生成 {count} 个可导入账号',
      dataImportLocalCacheSummary: '已处理本地缓存：导出 {exported} 个账号，跳过 {skipped} 个文件',
