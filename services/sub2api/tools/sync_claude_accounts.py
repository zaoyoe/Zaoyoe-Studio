#!/usr/bin/env python3
"""Export Claude Code OAuth accounts and import them into a sub2api admin panel."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export local Claude Code OAuth accounts and sync them directly to "
    ...
    parser.add_argument(
        ...
        action="append",
        dest="config_dirs",
        help="Claude config directory to export. Repeat to include multiple profiles.",
    )
    parser.add_argument(
        "--config-dir-glob",
        action="append",
        dest="config_dir_globs",
        help="Glob for multiple Claude config directories, such as '~/.claude-profiles/*'.",
    )
    parser.add_argument(
        "--oauth-suffix",
        help="Pass through to export_claude_accounts.py when using custom OAuth suffixes.",
    )
    parser.add_argument(
        "--disable-keychain",
        action="store_true",
        help="Skip macOS Keychain lookup and read only plaintext .credentials.json files.",
    )
    parser.add_argument(
        "--name-prefix",
        default="claude",
    ...
    parser.add_argument(
        "--notes-prefix",
        default="Imported from local Claude Code OAuth storage",
...
def ensure_base_url(args: argparse.Namespace) -> None:
    ...
    if not args.admin_base_url.strip():
        raise SystemExit(
            "Provide --admin-base-url or set $SUB2API_ADMIN_BASE_URL."
        )


def exporter_path() -> Path:
    return Path(__file__).resolve().with_name("export_claude_accounts.py")
...
def build_export_command(args: argparse.Namespace) -> list[str]:
    ...
    if args.oauth_suffix is not None:
        cmd.extend(["--oauth-suffix", args.oauth_suffix])
...
def post_import(args: argparse.Namespace, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    endpoint = import_endpoint(args.admin_base_url)
    request = urllib.request.Request(endpoint, data=request_body, method="POST")
...
def main() -> int:
    ...
    if attempted_accounts == 0:
        print("No exportable Claude accounts were found.", file=sys.stderr)
        return 1
    ...
    if args.dry_run:
        print(
            f"Prepared {attempted_accounts} Claude account(s) for import (dry run, no upload).",
            file=sys.stderr,
        )
        return 0
