...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, EndpointVideos) || path == "/videos" || strings.Contains(path, "/videos/"):
...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	case service.PlatformOpenAI:
		if inbound == EndpointEmbeddings || inbound == EndpointImagesGenerations || inbound == EndpointImagesEdits || inbound == EndpointVideos || inbound == EndpointVideosGenerations {
			return inbound
		}
