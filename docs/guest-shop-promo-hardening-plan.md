...
| 券被多用会不会亏钱 | **不会**，前提是每单折后仍有利润。券被用得越多 = 成交越多。真正需要防的是下面三条，而它们**与邮箱是否注册无关** |
...
#### 22.5.3 经济学上为什么原设计也站不住

「券被用越多越亏」这个前提本身不成立。只要每单折后仍有利润，券被用得越多 = 成交越多 = 利润越多。
真正会造成损失的是三件事，而它们**与邮箱是否注册毫无关系**（详见 §7.3.1）：

1. 营销预算被单点吃掉（机会成本）→ `guest_max_uses_per_identity` + `daily_budget_cny`
2. 库存被 pending 单占住（真实成交损失）→ `max_stock_hold_percent` + `max_pending_quantity_per_ip_product` + `promo_order_ttl_seconds`
3. 折后金额被算错或篡改（真实资损）→ DB CHECK 零元购 + `guest_min_payable_cny` + `guest_max_discount_percent ≤ 90` + L3 金额权威

**把风控做在结构上限上，而不是做在用户身份上——这是本节的全部要点。**
