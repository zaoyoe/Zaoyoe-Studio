...
| T4 | **身份漂移** | 清 cookie / 换 UA / 换 IP / 换邮箱，绕过 per-identity 上限 | 单个体获得无限配额 | §7.2 四因子并集计数 |
| T5 | **登出套利** | 注册账号用完 per-user 券后登出，用游客通道把同一张券再用一次 | 每用户限 1 次变成限 2 次，可规模化 | §7.3 contact_hash 命中注册账号即拒绝游客促销 |
...
- **P7 可计数身份**：游客必须有服务端签发的会话身份，配额按多因子并集计。
- **P8 可熔断、可回滚**：回滚 = 关开关。
...
| `buyer_contact_hash` | 高（真实身份） | 1 | 3 | 用券必填，最可信 |
