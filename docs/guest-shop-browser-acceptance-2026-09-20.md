...
> **历史快照声明（2026-09-21）：** 本文只记录 2026-09-20 本地夹具/IAB 的阶段性观察，不能代表当前生产状态或当前任务百分比。当前 2.1 进度为 **80%（4/5 阶段完成）**，生产 CN 单件指定 SKU 的邮箱+查询密码链路已由用户完成真实自测；当前剩余工作以 [`docs/guest-purchase-task-2.0.md`](./guest-purchase-task-2.0.md) §61.20 的阶段 5 清单为准。本文中的 `20%`、`NOT RUN` 和“下一阶段发布”均是当日历史快照，不得据此重复部署或要求固定观察窗口。
...
- 当日快照曾记录 Task 2.1 为 **20%（阶段 1/5 完成）**，下一阶段是默认关闭生产发布。该状态已由后续生产发布、指定 SKU 启用和用户自测覆盖；本文的 `NOT RUN/INCOMPLETE` 只约束对应浏览器质量声明或相关扩展功能，不倒推当前基础 SKU 已失败。Task 2.0 的旧 48% 同样仅是历史快照。
...
- 当日阶段状态快照：`Task 2.1=20%`、`阶段 1=complete`、`阶段 2=默认关闭生产发布 in_progress`；自动化为 `PASS-LOCAL`，完整真实浏览器矩阵仍为 `NOT RUN/INCOMPLETE`。当前不再使用该阶段指针；剩余矩阵作为阶段 5 的分层 backlog 保留。
