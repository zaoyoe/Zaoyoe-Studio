...
- [x] 2026-07-27：修复 `.98` 将 Prompt、视频和封面均完整但没有 X 来源的 Seedance Community 视频反复判为待详情，导致同两条记录循环清理、重采且进度长期不变的问题；`.99` 仅对媒体身份一致的自有 Community 视频放宽 X 来源要求，未改善的待重采修订不再重复入队，并优化当前卡片绑定、停止响应和弹窗统计口径；扩展版本升至 `0.1.50` / 采集器 `.99`。
- [x] 2026-07-27：修复 `.97` 每轮仓库预检都可能因提示词哈希查询未命中部分函数索引而触发 PostgreSQL statement timeout、导致全自动任务直接中断的问题；新迁移为各查询分支补齐索引谓词并跳过空媒体分支，`.98` 对预检/流式入队/占位清理增加瞬时错误退避重试，且仅在成功响应后提交已检查状态；扩展版本升至 `0.1.49` / 采集器 `.98`。
- [x] 2026-07-27：修复图片页把 `images.meigen.ai/uploads/...` 用户参考输入图拆成独立作品，导致同一卡片重复入队、详情补抓反复超时且“检查唯一作品”停滞的问题；`.97` 在候选与旧会话恢复层过滤参考图，优先选择带详情链接的完整卡片，并在查重/补抓前绑定真实 `/prompt/community_*` 身份后合并；扩展版本升至 `0.1.48` / 采集器 `.97`。
- [x] 2026-07-25：修复不同 X 链接或 Meigen 视频地址承载重复媒体时只靠来源/提示词无法预检的问题；`.96` 增加视频 `ETag + Content-Length`、缩略图 SHA-256 和 Worker 二进制 SHA-256 三级判重，历史 Gallery 视频从已有 R2 SHA 前缀与封面 `image_hash` 回填；扩展版本升至 `0.1.47` / 采集器 `.96`。
- [x] 2026-07-25：修复 Meigen 有 X 原帖地址时只按来源查重、忽略相同提示词的问题；`.95` 在预检同时校验来源与规范化提示词指纹，跨 3 条微批继续按 `prompt_hash` 判重，Worker 并发兜底重复立即清理并从 Import 隐藏，不再误入“待重新采集”；扩展版本升至 `0.1.46` / 采集器 `.95`。
- [x] 2026-07-24：修复 `.93` 在目标完成 `45/50` 时为 5 条未命中的待重采记录永久保留最后 5 个名额、令普通候选长期暂缓的问题；`.94` 在限定候选窗口仍未命中旧卡片后自动释放预留并立即补足目标，弹窗新增“重采预留”并将“候选暂缓”明确为“待后续候选”；扩展版本升至 `0.1.45` / 采集器 `.94`。
- [x] 2026-07-24：修复 `.92` 只要存在待重新采集记录就暂停全部普通新候选、导致检查到 244 条但仅接收 3 条的问题；`.93` 改为给待重采项预留目标名额，同时用其余容量继续接收普通卡片，并新增“候选暂缓”计数；扩展版本升至 `0.1.44` / 采集器 `.93`。
- [x] 2026-07-24：`.92` 将补抓失败但仍可恢复的候选持久化为“待重新采集”，恢复原批次时加载旧版 cleaned 未发布记录并受控回扫；服务端硬上限改按“已发布 + 未清理记录”计数，避免 Worker 清理发布记录后继续超额补量；视频带封面不再误判为缺图；扩展版本升至 `0.1.43` / 采集器 `.92`。
- [x] 2026-07-24：修复 `.90` 按滚动分段重置待详情基线、令目标 10 逐段累加到 50+ 的问题；客户端恢复按批次有效记录硬限流，补全失败占位项清理释放名额后再扫描替代卡片；服务端同步按 `max_items` 限制未清理记录，满额时只允许已有作品修订；扩展版本升至 `0.1.42` / 采集器 `.91`。
- [x] 2026-07-24：修复全自动采集把历史 `待详情` 计入实际入队目标后仅滚动 1 步就误报安全上限；目标改按可处理条目判断，失配待补记录继续保留但不占下一轮补量容量，采集器会继续接收延后卡片；终止提示仅在真实耗尽滚动/翻页预算时显示安全上限；扩展版本升至 `0.1.41` / 采集器 `.90`。
