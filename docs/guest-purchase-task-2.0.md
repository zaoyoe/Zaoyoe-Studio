...
- **任务 2.1 是当前唯一执行版本，当前总进度为 80%（4/5 阶段完成）。** 阶段 1「代码与合同」、阶段 2「默认关闭生产发布」、阶段 3「指定 SKU 游客启用」和阶段 4「用户主导自测」已完成；阶段 5「后续扩展与收口」尚未开始。
...
阶段 1 已在 2026-09-20 以发布候选代码和 `516/516` 游客商城自动化基线完成；阶段 2 已由 §61.14 的四链证据完成，阶段 3/4 的生产启用与用户自测结果见 §61.19–§61.20。当前进度 **80%**，剩余 **1/5 阶段，20%**；不再等待固定观察窗口。回滚演练仍是未执行的可选运维项目，不影响当前已确认的正常购买链路，但在扩大商品范围前应补做。
...
当前下一步是阶段 5 的范围决策与扩展排期：对 INTL、促销/优惠码、`quantity > 1`、quote/购物车、凭证增强、完整设备/辅助技术和 Admin Studio 运营能力逐项实施或明确 `deferred` 及重启条件。关闭指定 SKU后的回滚演练也尚未执行，需另行安排且不能与当前正常生产购买混为一谈；不得重复创建或支付历史未付款订单。
...
- [ ] 回滚演练：尚未实际关闭指定 SKU 验证“新单拒绝、在途已付款单继续履约或退款”。这是未执行的运维演练，不是当前正常购买链路的失败；在扩大白名单或发生异常前应补做。
- [ ] 阶段 5 扩展与收口：尚未开始，见下表。下表是待正式确认的初步延期清单，不是阶段 5 已完成的签署结果；还需要确定范围、责任人、重启条件并做最终归档。
...
因此，当前按五阶段计分为 **80%（4/5）**，剩余 **20%** 是阶段 5 的扩展与正式收口；当前指定「测试」SKU 的 CN 单件游客购买链路已完成并可继续由用户自行测试。
