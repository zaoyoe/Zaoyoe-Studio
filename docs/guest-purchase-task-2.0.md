...
| 5 | 后续扩展与收口 | 20% | **in_progress** | 已建立有限范围、角色责任、重启条件和证据产物；待完成最终运营归档及必要的 SKU 回滚演练 |
| **合计** |  | **100%** |  |  |
...
- [ ] 阶段 5 扩展与收口：已进入 `in_progress`，见下表。延期清单、责任角色、重启条件和最小证据产物已定稿；仍需完成最终运营归档及在明确授权窗口后的指定 SKU 回滚演练。
...
| 指定 SKU 回滚演练 | `pending_operator_window` | 商品开关负责人 + 支付/退款负责人 | 用户明确给出停用窗口，并确认没有需要继续支付的订单 | 关闭开关后新单 409；在途已付款单继续履约或进入退款；无数据库回滚 |

本阶段不执行：不新增 SQL、不从功能分支部署、不重复创建或支付历史订单、不在没有停用窗口的情况下关闭「测试」SKU、不把本地夹具或自动化测试写成生产证据。完成最终归档后，阶段 5 才可从 `in_progress` 更新为 `complete`；在此之前总进度保持 **80%**，不会循环等待固定时长。
