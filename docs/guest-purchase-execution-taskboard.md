...
> **历史状态声明（2026-09-21）：** 本文件是任务 1.0/早期 2.0 的冻结审计看板，下面的 1.0 百分比和阶段指针不代表当前执行状态。当前唯一执行版本是 [`docs/guest-purchase-task-2.0.md`](./guest-purchase-task-2.0.md) 中的任务 2.1；现行进度为 **80%（4/5 阶段完成）**，剩余阶段 5 的扩展与正式收口见该文件 §61.20。不要依据本文件启动重复部署、重复观察或重复下单。
...
>
> **2026-09-19 兼容说明：** 上述文件路径的内容版本已升级为任务 2.1；本冻结看板保留 2.0/1.0 历史进度，不作为 2.1 的执行源。

## 0. 进度口径与完成定义

- 任务 1.0 冻结进度：**82%**（数据库闸门已闭环；代码/契约测试完成；真实支付、设备视觉、运营演练仍是阻断项）
- **历史快照（已失效）：** 当时曾记录任务 2.1 为 **20%（阶段 1/5 complete，阶段 2 默认关闭生产发布 in_progress）**。该读数只保留用于追溯，不再参与当前计分，也不构成新的门禁。本文件冻结进度仍为 **82%**，不回写任何后续版本百分比到下方 1.0 checkbox。
- 当前阶段指针请以任务 2.1 §1、§61.20 和 §61.21 为准：阶段 1–4 已完成，阶段 5 已进入 `in_progress`，但总进度仍为 80%；完整浏览器矩阵和 INTL/促销/多件等扩展证据按功能分层保留。
- **2026-09-21 收口更新：** 指定 SKU 回滚演练与运营归档已完成，归档见 [`docs/guest-shop-stage5-rollback-archive.md`](./guest-shop-stage5-rollback-archive.md)；阶段 5 仍保持 `in_progress`，下一步按用户顺序进入促销及后续扩展，不把延期工作包误记为完成。
...
> **历史标记（1.0 冻结记录）：** 本节的跨设备取货凭证描述记录的是旧版实现事实，不是当前 2.1 的买家查询入口。当前跨设备查询统一使用 `/guest-orders.html` 的邮箱 + 查询密码；服务端 claim proof 仅用于履约授权。
...
- [x] 在现有 `shop.html`/`js/shop-client.js` 购买弹窗中增加游客入口，保留登录用户积分购买路径。
      （2026-09-17 起入口形态变更：独立的「游客购买」按钮已按用户要求移除，改为**仅未登录**时把游客现金直付
      并入主按钮，文案切「立即购买 / Buy now」，并隐藏数量与优惠码 stage；`js/guest-shop-client.js` 只通过
      `window.GuestShopCheckout` 桥接暴露可用性，登录态判断留在 `js/shop-client.js`。详见
      `docs/guest-purchase-task-2.0.md` 第 58 节。）
...
- 游客购买不要求登录；登录用户行为和 UI 不回归。
  （2026-09-17 口径修正：原「登录用户也能看到游客购买按钮」不再成立，这是**有意的**入口收缩。
  现在的不变量是「登录用户主按钮恒为『兑换』且走积分流程」；代价是积分不足的登录用户暂时没有现金出路，
  是否补入口属未决产品决策。）
