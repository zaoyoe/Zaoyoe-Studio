...
- 当前执行与百分比只以任务 2.1 五阶段表为准：当前 **20%（阶段 1/5 complete，阶段 2 默认关闭生产发布 in_progress）**。旧 2.0 的 **48%** 以及 D3 明细均冻结为历史证据，不再参与 2.1 计分或阻塞默认关闭发布。本文件冻结进度仍为 **82%**，不回写任何后续版本百分比到下方 1.0 checkbox。
- 当前阶段指针：`任务 2.1 / 阶段 2 默认关闭生产发布 in_progress；完整浏览器矩阵和 INTL/促销/多件等扩展证据按功能分层保留，不构成阶段 2 的一揽子前置`
