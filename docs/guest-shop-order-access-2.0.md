...
| P7a | 拒绝内置 Top-2000 弱口令黑名单（静态列表，**不联网**，展开后 11052 条） | 含 `Password1!`、`Passw0rd!` 这类「满足四类但人人都在用」的口令——**P2 挡不住它们，P7 才挡得住**。查表前先做 leet 反折（`0→o` `1→i` `3→e` `4→a` `5→s` `7→t` `8→b` `@→a` `$→s` `!→i` `+→t`）并剥掉末尾最多 3 个**数字/标点**装饰（**从不剥字母**，「含常见词」是 P7b-① 的职责），因此 `P@ssw0rd1`、`Admin123$`、`Qwerty2!` 也命中。黑名单只枚举每个核心词的**全量** leet 折叠形（`password` → `p455w0rd`）；部分折叠形（`passw0rd`）靠运行时反折覆盖——枚举全部子集是指数级的，收益为零 |
...
| P8 | 拒绝与 `order_no`、站点域名**相同或互相包含** | 域名额外按**标签**匹配：配置 `nightjar.shop` 时密码含 `nightjar` 即拒——没人会把 TLD 打进密码框，而静态黑名单不可能预先知道每个运营方的品牌。长度 <4 的标签（`www` / `co` / `xyz` / `shop`）只在完全相等时才命中，避免误杀；`order_no` 的最小长度阈值为 6 |
...
> **求值顺序是契约的一部分**：`P1 → P3 → P4 → P2a-d → P9 → P10 → P6 → P7a → P7b → P8`。
> 先判便宜的（长度、字符集）再判贵的（查表、正则），且**返回哪条规则**决定了前端文案，
> 因此测试对规则编号做精确断言。两个已知后果：
> ① `Abcd1234!` 报的是 **P7a**（`abcd` 在黑名单里）而不是 P7b-③，尽管它同时命中结构规则；
> ② `Xfatherkey9#` 报的是 **P7b-③**（结构）而不是 P8，尽管 `fatherkey` 也在黑名单核心词里。
> 顺序变动必须同步改 §16.1 的用例。
...
- 密码策略 **P1–P10** 每条规则的通过/拒绝用例，含「密码包含邮箱本地部分」。
- **四类齐全（P2）**：`Xk9#mQ2$zW` 通过（**不能**用 `Abcd123!` 当通过样例——它满足四类，
  但 `abcd123` 在黑名单里，会被 P7a 拒；本文的规范通过样例统一为 `Ab3!xY9#`）；
  `abcd123!`（无大写）/`ABCD123!`（无小写）/`Abcdefg!`（无数字）/`Abcd1234`（无标点）
  各拒绝且错误码分别为 P2a/P2b/P2c/P2d。
- **P7a 黑名单**：`Password1!`、`Passw0rd!`、`P@ssw0rd1`、`Admin123$` 满足四类但必须被拒
  （后两个靠「剥末尾装饰 + leet 反折」命中）；`GUEST_QUERY_PASSWORD_DENYLIST.size > 2000`。
- **P7b 模式**：`Qwer1234!`（键盘序列）、`Xadm1n#9kQ`（词干）、`MyP4ssw0rd#`（词干）、
  `Xmnop9#kQ`（单调字母序列）/ `X9876#kQm`（键盘行 `1234567890` 的反向窗口）、`Kzvqm93!`（结构）全部拒绝；
  `Qwer1234!` 的反向 `!4321rewQ` 同样拒绝。
  注意 `Abcd1234!` / `Test123!` / `Admin123!` / `Iloveyou1!` 报的是 **P7a**（求值顺序在前），
  断言时不要写死成 P7b。
- **P9 / P10**：`Aaaa1111!!!!` 拒（连续重复）；`ZaAaA9!x` 中 `aAAa` 大小写不敏感判定为
  连续 4 个 `a` 而拒；`Xk9####mQ2` 拒（`####` 连续 4 次）而 `Xk9###mQ2z` 通过（3 次允许）；
...
- **「帮我生成」**：产出必然通过 P1–P10（拒绝采样）；不含易混字符 `0Oo1lI|`；长度恒为 12；
  两次生成不相同；500 次采样无重复且全部合规。
- `X-Guest-Order-Credential` 解析：合法、非法 base64、缺 `\n`、超长 email、超长 password，
  以及**非规范 base64url 再编码**（载荷字节数必须 ≡1 mod 3 才存在这种变体，
  例如 `a@b.co\n` + 9 位密码 = 16 字节；末尾字符的 4 个 slack bit 允许 60 种等价编码，
  全部拒为 `guest_credential_malformed`）。
  反之，**字符集非法但结构合法**的密码不是 400：它不可能被存过，必须流到校验器
  返回与「密码错误」同一个统一 403，否则白送一个区分信号。
- **query 形式凭证必须被拒**（对齐独角 `guest_auth_test.go`）。
- 锁定阶段机：5 次失败 → 15min → 30min → 24h → 人工；锁定期间不跑 scrypt。
- 时间侧信道：`unknown_email` 与 `bad_password` 都执行**恰好一次** scrypt
  （spy 替换 `crypto.scryptSync` 断言调用次数），且 dummy hash 跑在当前成本参数上、
  永不校验通过任何真实密码。
...
| **A0-verify-INV** | ✅ 已完成 | `待回填` | 行清单冻结补齐 + 行数勘误：`tests/guest-shop-verify-probe-contract.test.js` 的「verify 行清单冻结」断言此前只覆盖 `20260920`(11) 与 `20260922`(7)，**A1b 不在册**，导致文档把步骤 2 记成 5 行、三步总数记成 23 行；现补上 `checkNames(SOURCES.a1bVerify)` 的 6 项断言，三个 verify 全部在册（11 + 6 + 7 = **24 行**）。用户实机输出的步骤 2 六行逐行归档到 `docs/guest-shop-promo-evidence.md` §1.7，§1.1 与本文档三处计数同步改为 24。变异验证：把 verify 里的 `upsert_fn_grants` 改名 → 该测试立即红（10 pass / 1 fail），还原后 `git diff` 为空。**迁移与 verify SQL 一行未改、无需重跑。** 全量 `npm run test:security`：**3333 通过 / 0 失败**（条数不变，新增的是既有测试内的断言） |
...
>
> 每步之后跑对应的 verify 脚本（`20260920_verify_guest_shop_buyer_credentials.sql` /
> `20260921_verify_guest_shop_buyer_group_upsert.sql` / `20260922_verify_guest_shop_access_resets.sql`），
> **全部 PASS** 才允许打开开关。启用前置条件见 §15.3 与 §16.4。
>
> **实机执行记录（2026-09-18，由你在目标 Supabase 执行，Codex 未执行任何 SQL）**：三个迁移均已落库，**三步校验现已全部通过（24 行 PASS / 0 FAIL）**——`20260920_verify` 11/11、`20260921_verify` 6/6、`20260922_verify` 7/7。`20260920_verify` 首轮曾出现 1 行 FAIL（`password_format_pins_scrypt_and_norm_version`），诊断为**探针自身缺陷**而非迁移缺陷（D-10）；探针修复后**同日复跑即 11/11 全 PASS**，迁移一行未改、未重跑。逐行结果与诊断归档在 `docs/guest-shop-promo-evidence.md` §1（步骤 1 见 §1.5、步骤 2 见 §1.7、步骤 3 见 §1.2、假 FAIL 诊断见 §1.3）。开关仍未打开。
>
