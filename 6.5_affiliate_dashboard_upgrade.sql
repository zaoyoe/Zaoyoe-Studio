-- ============================================
-- 6.5 推广旅程面板 + 奖励详情热修复
-- 建议先执行 6.4_decimal_points_support.sql
-- 作用：
-- 1. 推广页返回被邀请人名单、阶段、贡献和奖励汇总
-- 2. 订单里推广奖励可反查到邀请对象与关联订单/首充
-- 3. 首充时也能激活待发放的拉新奖励
-- ============================================
...
DROP FUNCTION IF EXISTS public.fn_recharge_points(UUID, NUMERIC, NUMERIC, TEXT, TEXT);
...
-- 默认只允许本人读取邀请统计/返佣详情；管理员和 service_role 可按需查看他人
-- ============================================
...
DROP FUNCTION IF EXISTS public.fn_recharge_points(UUID, NUMERIC, NUMERIC, TEXT, TEXT, VARCHAR);
