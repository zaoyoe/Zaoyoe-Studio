...
CREATE OR REPLACE FUNCTION fn_daily_checkin_v2(
    p_user_id UUID,
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        WHERE user_id = p_user_id AND site = p_site AND check_date = current_date
    ...
    -- 插入签到记录
    INSERT INTO user_checkins (user_id, site, check_date) VALUES (p_user_id, p_site, current_date);

    -- 检查本月全勤 (当月总天数 vs 当月已签到天数)
    v_days_in_month := EXTRACT(DAY FROM (date_trunc('month', current_date) + interval '1 month - 1 day'));
    SELECT COUNT(*) INTO v_checked_in_month FROM user_checkins 
    WHERE user_id = p_user_id AND site = p_site AND date_trunc('month', check_date) = date_trunc('month', current_date);
    ...
    IF v_total_reward > 0 THEN
        ...
        VALUES (p_user_id, p_site, 0, v_total_reward)
        ON CONFLICT (user_id, site) DO UPDATE SET
            bonus_balance = points_balance.bonus_balance + v_total_reward,
            updated_at = NOW();

        INSERT INTO points_ledger (user_id, site, amount, reason, reference_id)
        VALUES (p_user_id, p_site, v_total_reward, 'daily_checkin', 'CHK_' || to_char(current_date, 'YYYYMMDD'));
    END IF;
...
-- 4. 补签逻辑
CREATE OR REPLACE FUNCTION fn_makeup_checkin(
    p_user_id UUID,
    p_site TEXT,
    p_date DATE,
...
BEGIN
    ...
    -- 检查是否已签
    IF EXISTS(SELECT 1 FROM user_checkins WHERE user_id = p_user_id AND site = p_site AND check_date = p_date) THEN
    ...
    -- 执行补签 (注意：补签不给当日基础积分，但会影响连续天数和全勤)
    INSERT INTO user_checkins (user_id, site, check_date, is_makeup, makeup_method) 
