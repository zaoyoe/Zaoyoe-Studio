-- Support date range filtering in inventory management
DROP FUNCTION IF EXISTS fn_admin_list_inventory(UUID, VARCHAR, VARCHAR, INT, INT);
DROP FUNCTION IF EXISTS fn_admin_list_inventory(UUID, VARCHAR, VARCHAR, INT, INT, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE);
...
BEGIN
    ...
    -- Get stats (filtered by product, but usually stats are global or per product)
    -- We keep stats filtering minimal (product only) to show overview, 
    -- Or should stats obey all filters? Usually stats are "Overview".
    -- Let's keep existing behavior: status stats by product.
