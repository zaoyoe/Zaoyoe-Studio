-- Promote retention cohorts to real business activity first while
-- preserving login history as an automatic fallback when the selected
-- window has no effective business events.
...
BEGIN
    ...
    user_activity AS (
        SELECT DISTINCT
            ...
            date_trunc('week', scoped_activity.created_at AT TIME ZONE 'Asia/Shanghai')::date as activity_week
        FROM (
            SELECT
                ue.user_id,
                ue.created_at
            FROM public.user_events ue
            WHERE v_has_business_events
            ...
            SELECT
                ...
                ulh.created_at
            FROM public.user_login_history ulh
            WHERE NOT v_has_business_events
              AND get_local_date(ulh.created_at) BETWEEN v_start_date AND v_end_date
    ...
    SELECT
        ...
        NOT v_has_business_events AS is_proxy_metric,
        CASE
            WHEN v_has_business_events
                THEN 'site_attributed_cohort_effective_business_activity'
            ELSE 'site_attributed_cohort_login_activity'
        END::TEXT AS metric_basis,
        CASE
            WHEN v_has_business_events
                THEN '首站点归因 cohort + 真实业务回访'
            ELSE '首站点归因 cohort + 登录回访代理口径'
        END::TEXT AS metric_label
    FROM retention;
END;
