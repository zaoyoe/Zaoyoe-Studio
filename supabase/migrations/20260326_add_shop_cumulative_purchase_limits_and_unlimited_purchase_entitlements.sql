-- ============================================
-- Add cumulative shop purchase limits and unlimited-purchase entitlements
-- - per-user per-product rolling caps
-- - per-account lifetime caps
-- - user-level unlimited purchase override
...
SET purchase_limit_24h_quantity = NULL
WHERE purchase_limit_24h_quantity IS NOT NULL
  AND purchase_limit_24h_quantity < 1;

UPDATE public.shop_products
SET purchase_limit_window_minutes = NULL
WHERE purchase_limit_window_minutes IS NOT NULL
  AND purchase_limit_window_minutes < 1;

UPDATE public.shop_products
SET purchase_limit_window_quantity = NULL
WHERE purchase_limit_window_quantity IS NOT NULL
  AND purchase_limit_window_quantity < 1;

UPDATE public.shop_products
SET per_account_purchase_limit = NULL
WHERE per_account_purchase_limit IS NOT NULL
  AND per_account_purchase_limit < 1;

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_purchase_limit_24h_quantity_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_purchase_limit_24h_quantity_check
    CHECK (
        purchase_limit_24h_quantity IS NULL
        OR purchase_limit_24h_quantity >= 1
    );

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_purchase_limit_window_minutes_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_purchase_limit_window_minutes_check
    CHECK (
        purchase_limit_window_minutes IS NULL
        OR purchase_limit_window_minutes >= 1
    );

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_purchase_limit_window_quantity_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_purchase_limit_window_quantity_check
    CHECK (
        purchase_limit_window_quantity IS NULL
        OR purchase_limit_window_quantity >= 1
    );

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_purchase_limit_window_pair_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_purchase_limit_window_pair_check
    CHECK (
        (purchase_limit_window_minutes IS NULL AND purchase_limit_window_quantity IS NULL)
        OR (purchase_limit_window_minutes IS NOT NULL AND purchase_limit_window_quantity IS NOT NULL)
    );

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_per_account_purchase_limit_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_per_account_purchase_limit_check
    CHECK (
        per_account_purchase_limit IS NULL
        OR per_account_purchase_limit >= 1
    );

COMMENT ON COLUMN public.shop_products.purchase_limit_24h_quantity IS 'Optional per-user per-product rolling 24-hour purchase cap. NULL means no 24-hour cap.';
COMMENT ON COLUMN public.shop_products.purchase_limit_window_minutes IS 'Optional rolling time window in minutes for per-user per-product purchase throttling.';
COMMENT ON COLUMN public.shop_products.purchase_limit_window_quantity IS 'Optional per-user per-product purchase cap inside purchase_limit_window_minutes.';
COMMENT ON COLUMN public.shop_products.per_account_purchase_limit IS 'Optional lifetime per-user per-product purchase cap across all sites.';

CREATE INDEX IF NOT EXISTS idx_shop_orders_user_product_created_at
...
COMMENT ON TABLE public.user_purchase_entitlements IS 'Per-user shop purchase entitlements and bypass flags managed by super admins.';
COMMENT ON COLUMN public.user_purchase_entitlements.unlimited_shop_purchases IS 'When true, the user bypasses single-order and cumulative shop purchase caps.';

ALTER TABLE public.user_purchase_entitlements ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own purchase entitlements" ON public.user_purchase_entitlements;
CREATE POLICY "Users can view own purchase entitlements"
ON public.user_purchase_entitlements FOR SELECT
USING (
    auth.uid() = user_id
    OR public.is_super_admin()
);

DROP POLICY IF EXISTS "Super admins manage purchase entitlements" ON public.user_purchase_entitlements;
CREATE POLICY "Super admins manage purchase entitlements"
ON public.user_purchase_entitlements FOR ALL
USING (public.is_super_admin())
WITH CHECK (public.is_super_admin());

REVOKE ALL ON public.user_purchase_entitlements FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_purchase_entitlements TO authenticated, service_role;
