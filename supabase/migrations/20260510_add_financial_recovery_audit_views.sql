-- Financial recovery audit views for payment, points, and shop inventory flows.
-- These views are read-only and admin-scoped. They do not change purchase,
-- payment, refund, or points mutation behavior.
...
GRANT SELECT ON public.admin_payment_order_recovery_audit_view TO authenticated;
GRANT SELECT ON public.admin_points_balance_recovery_audit_view TO authenticated;
GRANT SELECT ON public.admin_shop_inventory_recovery_audit_view TO authenticated;
GRANT SELECT ON public.admin_financial_recovery_audit_summary_view TO authenticated;

COMMENT ON VIEW public.admin_payment_order_recovery_audit_view IS 'Admin-only payment recovery audit view linking paid orders, redemption codes, and ledger credits.';
COMMENT ON VIEW public.admin_points_balance_recovery_audit_view IS 'Admin-only points recovery audit view comparing points_balance totals against immutable points_ledger sums.';
COMMENT ON VIEW public.admin_shop_inventory_recovery_audit_view IS 'Admin-only shop recovery audit view linking orders, inventory item state, and ledger debits.';
COMMENT ON VIEW public.admin_financial_recovery_audit_summary_view IS 'Admin-only rollup of payment, points, and shop recovery audit findings.';
