-- Split shop marketing pricing fields by site so CN tier/flash pricing no longer leaks into INTL.

ALTER TABLE public.shop_products
    ADD COLUMN IF NOT EXISTS quantity_rules_intl JSONB DEFAULT NULL,
    ADD COLUMN IF NOT EXISTS flash_sale_price_intl NUMERIC(12,2) DEFAULT NULL,
    ADD COLUMN IF NOT EXISTS flash_sale_end_intl TIMESTAMPTZ DEFAULT NULL;

COMMENT ON COLUMN public.shop_products.quantity_rules IS 'CN shop tiered pricing rules.';
COMMENT ON COLUMN public.shop_products.quantity_rules_intl IS 'INTL shop tiered pricing rules; NULL means no INTL tier pricing.';
COMMENT ON COLUMN public.shop_products.flash_sale_price IS 'CN shop flash-sale unit price.';
COMMENT ON COLUMN public.shop_products.flash_sale_price_intl IS 'INTL shop flash-sale unit price; NULL means no INTL flash sale.';
COMMENT ON COLUMN public.shop_products.flash_sale_end IS 'CN shop flash-sale end timestamp.';
COMMENT ON COLUMN public.shop_products.flash_sale_end_intl IS 'INTL shop flash-sale end timestamp; NULL means no INTL flash sale.';
...
CREATE TABLE IF NOT EXISTS public.agent_prices (
    ...
    site VARCHAR(16) DEFAULT 'cn' NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (agent_id, product_id, site)
);

ALTER TABLE public.agent_prices
    ADD COLUMN IF NOT EXISTS site VARCHAR(16) DEFAULT 'cn' NOT NULL;

UPDATE public.agent_prices
...
ALTER TABLE public.agent_prices
    DROP CONSTRAINT IF EXISTS agent_prices_site_check;

ALTER TABLE public.agent_prices
    ADD CONSTRAINT agent_prices_site_check CHECK (site IN ('cn', 'intl'));

ALTER TABLE public.agent_prices
    DROP CONSTRAINT IF EXISTS agent_prices_pkey;

ALTER TABLE public.agent_prices
    ADD CONSTRAINT agent_prices_pkey PRIMARY KEY (agent_id, product_id, site);

CREATE INDEX IF NOT EXISTS idx_agent_prices_site ON public.agent_prices(site);

COMMENT ON COLUMN public.agent_prices.site IS 'Site-specific agent storefront price scope.';

CREATE OR REPLACE FUNCTION public.fn_resolve_site_scoped_system_config(
    p_key TEXT,
...
BEGIN
    ...
    IF COALESCE(v_config->>'__site_scoped', '') <> 'true' THEN
        RETURN v_config;
    END IF;

    v_sites := CASE WHEN jsonb_typeof(v_config->'sites') = 'object' THEN v_config->'sites' ELSE '{}'::JSONB END;
    IF v_sites ? v_site THEN
        RETURN v_sites->v_site;
    END IF;

    IF v_config ? 'default' THEN
        RETURN v_config->'default';
    END IF;
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        WHERE user_id = v_effective_user_id AND site = v_site AND check_date = current_date
    ...
    v_config := public.fn_resolve_site_scoped_system_config('checkin_system', v_site);
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM public.user_checkins WHERE user_id = v_effective_user_id AND site = v_site AND check_date = v_check_date) THEN
    ...
    VALUES (v_effective_user_id, v_site, current_date);
    ...
    WHERE user_id = v_effective_user_id AND site = v_site AND date_trunc('month', check_date) = date_trunc('month', current_date);
    ...
    IF v_total_reward > 0 THEN
        ...
        VALUES (v_effective_user_id, v_site, 0, v_total_reward)
        ...
        VALUES (v_effective_user_id, v_site, v_total_reward, 'daily_checkin', 'CHK_' || to_char(current_date, 'YYYYMMDD'));
    END IF;

    SELECT COALESCE(total_balance, 0)
    INTO v_new_balance
    FROM public.points_balance
    WHERE user_id = v_effective_user_id AND site = v_site;
...
BEGIN
    ...
    IF EXISTS(SELECT 1 FROM public.user_checkins WHERE user_id = v_effective_user_id AND site = v_site AND check_date = p_date) THEN
    ...
    IF p_method = 'points' THEN
        ...
        IF v_cost > 0 THEN
            ...
            INSERT INTO public.points_ledger (user_id, site, amount, reason, reference_id)
            VALUES (v_effective_user_id, v_site, -v_cost, 'makeup_checkin_cost', 'MKP_COST_' || to_char(p_date, 'YYYYMMDD'));
        END IF;
    ...
    VALUES (v_effective_user_id, v_site, p_date, true, p_method);
...
GRANT EXECUTE ON FUNCTION public.fn_resolve_site_scoped_system_config(TEXT, TEXT) TO authenticated, service_role;
