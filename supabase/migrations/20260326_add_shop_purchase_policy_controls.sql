-- ============================================
-- Add shop purchase policy controls
-- - per-product max purchase quantity
-- - discount-code allow_zero_total guardrail
-- - enforce both rules in discount preview and purchase RPCs
-- ============================================
...
SET max_purchase_quantity = NULL
WHERE max_purchase_quantity IS NOT NULL
  AND max_purchase_quantity < 1;

UPDATE public.shop_products
SET max_purchase_quantity = 99
WHERE max_purchase_quantity IS NOT NULL
  AND max_purchase_quantity > 99;

ALTER TABLE public.shop_products
    DROP CONSTRAINT IF EXISTS shop_products_max_purchase_quantity_check;

ALTER TABLE public.shop_products
    ADD CONSTRAINT shop_products_max_purchase_quantity_check
    CHECK (
        max_purchase_quantity IS NULL
        OR (max_purchase_quantity >= 1 AND max_purchase_quantity <= 99)
    );

COMMENT ON COLUMN public.shop_products.max_purchase_quantity IS 'Optional per-product purchase cap. NULL falls back to the global 99-item ceiling.';

ALTER TABLE public.discount_codes
    ADD COLUMN IF NOT EXISTS allow_zero_total BOOLEAN DEFAULT false;

UPDATE public.discount_codes
SET allow_zero_total = false
WHERE allow_zero_total IS NULL;

ALTER TABLE public.discount_codes
    ALTER COLUMN allow_zero_total SET DEFAULT false;

ALTER TABLE public.discount_codes
    ALTER COLUMN allow_zero_total SET NOT NULL;

COMMENT ON COLUMN public.discount_codes.allow_zero_total IS 'When false, the discount code cannot reduce an order total to zero.';
