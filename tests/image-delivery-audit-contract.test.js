...
test('image upload edge function enforces server-side type size and rate guardrails', () => {
    ...
    assert.match(source, /const ADMIN_UPLOAD_PERMISSIONS: Partial<Record<UploadType, string>>/);
    assert.match(source, /product: 'shop\.manage'/);
    assert.match(source, /poster: 'discounts\.manage'/);
    assert.match(source, /homepage: REQUIRED_HOMEPAGE_UPLOAD_PERMISSION/);
    ...
    assert.match(source, /function assertSafeRemoteImageUrl/);
    assert.match(source, /function assertImageMagicBytes/);
    ...
    assert.match(source, /requireAdminImageUploadPermission\([\s\S]*requiredAdminPermission/);
