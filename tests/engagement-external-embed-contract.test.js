...
test('external engagement embed can mount the robot runtime on gongyi pages', () => {
    ...
    assert.match(embed, /CHAT_WIDGET_LOADER_SRC\s*=\s*'js\/chat-widget-loader\.js\?v=20260505_CHAT_USER_ACTIVITY_1'/);
    assert.match(embed, /CHAT_WIDGET_STYLE_SRC\s*=\s*'css\/chat-widget\.css\?v=20260505_ENGAGEMENT_ENTRY_1'/);
...
test('chat widget loader resolves assets from external embed asset base', () => {
    ...
    assert.match(loader, /CHAT_WIDGET_STYLE_SRC\s*=\s*'css\/chat-widget\.css\?v=20260505_ENGAGEMENT_ENTRY_1'/);
