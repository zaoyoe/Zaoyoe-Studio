...
test('site layout admin editor and runtime wire footer contacts without stale cache lock-in', () => {
    ...
    assert.match(runtimeSource, /if \(navigator\.clipboard\?\.writeText\) \{[\s\S]*?return navigator\.clipboard\.writeText\(value\)\.catch\(\(clipboardError\) => \{[\s\S]*?if \(shouldPreferLegacySupportCopy\(\)\) \{[\s\S]*?return Promise\.reject\(clipboardError\);[\s\S]*?return legacyCopySupportText\(value\);/);
    assert.match(runtimeSource, /if \(shouldPreferLegacySupportCopy\(\)\) \{[\s\S]*?return Promise\.reject\(new Error\('复制失败'\)\);[\s\S]*?return legacyCopySupportText\(value\);/);
    ...
    assert.doesNotMatch(runtimeSource, /return action === 'copy' \|\| action === 'chat' \|\| action === 'detail';/);
    assert.match(runtimeSource, /return action === 'chat' \|\| action === 'detail';/);
    ...
    assert.match(runtimeSource, /copySupportText\(copyValue\)[\s\S]*?\.then\(\(\) => \{[\s\S]*?setSupportLinkFeedback\(link, 'copied', getSupportFeedbackText\('copy-success-short', copyValue\)\);[\s\S]*?\}\)[\s\S]*?\.catch\(\(error\) => \{[\s\S]*?setSupportLinkFeedback\(link, 'error', getSupportFeedbackText\('copy-error-short', copyValue\)\);[\s\S]*?error\?\.message === '没有可复制的内容'[\s\S]*?\}\);/);
    const footerCopyActionMatch = runtimeSource.match(/if \(action === 'copy'\) \{[\s\S]*?copySupportText\(copyValue\)[\s\S]*?\.catch\(\(error\) => \{([\s\S]*?)\}\);[\s\S]*?return;[\s\S]*?\}/);
    assert.ok(footerCopyActionMatch, 'footer copy action handles copy failures explicitly');
    assert.doesNotMatch(footerCopyActionMatch[1], /setSupportLinkFeedback\(link, 'copied'/);
    assert.match(footerCopyActionMatch[1], /setSupportLinkFeedback\(link, 'error', getSupportFeedbackText\('copy-error-short', copyValue\)\);/);
