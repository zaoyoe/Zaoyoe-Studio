...
test('Meigen collector extension can collect, download, and stage import payloads', () => {
    ...
    assert.match(content, /while \(streamStageState\.processableCount < maxItems[\s\S]*&& !automationJob\.stopRequested\)/);
    ...
    assert.match(content, /checkedKeys\.has\(key\) && !isStreamItemRevisionImproved\(checkedRevisions\.get\(key\), revision\)/);
    ...
    assert.match(popup, /历史顽固 \$\{status\.persistentFailures\}/);
    ...
    assert.match(collector, /isVideoCollectionUrl\(options\.baseUrl \|\| baseUrl\) && !videoSources\.length/);
