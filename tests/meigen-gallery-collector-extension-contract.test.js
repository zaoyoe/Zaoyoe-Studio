...
test('Meigen collector extension can collect, download, and stage import payloads', () => {
    ...
    assert.match(content, /scrollOneViewport\(\)/);
    assert.doesNotMatch(content, /window\.scrollTo\(\{ top: previousSnapshot\.height/);
    assert.match(content, /function getVisibleGalleryMediaState/);
    assert.match(content, /collectionRoot\.querySelectorAll\('img, video'\)/);
    assert.match(content, /pendingMedia/);
    ...
    assert.match(collector, /function getActiveCollectionPanel/);
    assert.match(collector, /const collectionRoot = getActiveCollectionPanel\(documentRef\) \|\| documentRef/);
    assert.match(collector, /videoCollectionContext && !videoSources\.length/);
    assert.match(collector, /function collectStructuredVideoUrls/);
    assert.match(collector, /function filterVideoSourcesByIdentity/);
    assert.match(collector, /function isVideoCollectionUrl/);
    assert.match(collector, /structuredCandidate\?\.videoUrls/);
    ...
    assert.match(content, /extractMediaPromptBlocks\?\.\(node\)/);
    ...
    assert.match(content, /function getCardArtworkClickTarget/);
    assert.match(content, /function getItemMediaUrlSet/);
    assert.match(content, /document\.querySelectorAll\('img, video'\)/);
    assert.match(content, /function activateCardDetailTarget/);
    assert.match(content, /target\.click\(\)/);
    assert.match(content, /const rawValue = String\(value \|\| ''\)\.trim\(\);/);
    assert.match(content, /if \(!rawValue\) return '';/);
    assert.match(content, /function detailDialogMatchesTarget/);
    assert.match(content, /function ensureDetailViewClosed/);
    assert.match(content, /function getCloseControlLabel/);
    assert.match(content, /return values\[0\] \|\| ''/);
    assert.match(content, /DETAIL_DIALOG_SELECTOR/);
    assert.match(content, /scope\.querySelector\?\.\('\[draggable="true"\]'\)/);
    assert.match(content, /if \(exactLink\) return exactLink;[\s\S]{0,400}const draggableTarget[\s\S]{0,400}if \(draggableTarget\) return draggableTarget;[\s\S]{0,400}const artworkTarget/);
    assert.match(content, /if \(Array\.isArray\(item\?\.video_sources\) && item\.video_sources\.length\) continue/);
    assert.match(collector, /options\.detailOnly\s*\|\|\s*expectedDetailUrl/);
    ...
    assert.match(content, /await streamCheckpointForItem\(item\)/);
    assert.match(content, /await streamCheckpointForItem\(\{ source_page_url: url \}\)/);
