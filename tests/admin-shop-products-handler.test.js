...
const SHOP_PRODUCT_SKU_SELECT = 'id, product_id, sku_code, sku_name, spec_values, inventory_sku_id, inventory_source_sku_ids, inventory_source_sku_ids_intl, manual_delivery, price_points, price_points_intl, quantity_rules, quantity_rules_intl, is_default, is_active, stock_count, sort_order';
const SHOP_PRODUCT_SKU_SELECT_WITHOUT_INTL_INVENTORY_SOURCE_LIST = 'id, product_id, sku_code, sku_name, spec_values, inventory_sku_id, inventory_source_sku_ids, manual_delivery, price_points, price_points_intl, quantity_rules, quantity_rules_intl, is_default, is_active, stock_count, sort_order';
...
test('shop products handler falls back while intl inventory source column is warming up', async () => {
    await withShopProductsHandler({
        queryResults: {
            shop_products: [
                {
            ...
            shop_product_skus: [
                {
                    data: null,
                    error: { message: 'Could not find the inventory_source_sku_ids_intl column of shop_product_skus in the schema cache' }
                },
                {
                    data: [
                        {
        ...
        assert.equal(state.queryCalls[2]?.operations?.[0]?.args?.[0], SHOP_PRODUCT_SKU_SELECT_WITHOUT_INTL_INVENTORY_SOURCE_LIST);
    });
});
...
test('shop products handler falls back while shared inventory column is warming up', async () => {
    await withShopProductsHandler({
        ...
        assert.equal(state.queryCalls[1]?.operations?.[0]?.args?.[0], SHOP_PRODUCT_SKU_SELECT);
        assert.equal(state.queryCalls[2]?.operations?.[0]?.args?.[0], SHOP_PRODUCT_SKU_SELECT_WITHOUT_INTL_INVENTORY_SOURCE_LIST);
        assert.equal(state.queryCalls[3]?.operations?.[0]?.args?.[0], SHOP_PRODUCT_SKU_SELECT_WITHOUT_INVENTORY_SOURCE_LIST);
    });
});
