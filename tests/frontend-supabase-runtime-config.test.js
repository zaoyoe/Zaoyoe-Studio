...
test('admin ops alert controls expose delegated settings actions and runtime wiring', () => {
    ...
    assert.equal(
        adminStudioSource.includes('js/admin-workbench.js?v=20260612_OPS_ALERT_PREVIEW_SITE_2'),
        true,
    ...
    assert.equal(
        adminConfigSource.includes("const writableSite = requireWritableAdminSettingsSite('发送站外告警联调测试');"),
        true,
        'admin-config.js should require a concrete site before sending ops alert preview tests'
    );
    assert.equal(
        adminConfigSource.includes('site: writableSite,'),
        true,
        'admin-config.js should submit ops alert preview tests with the selected writable site'
    );
    assert.equal(
...
test('ticket admin surfaces user email in search and list rendering', () => {
    ...
    assert.equal(adminStudioSource.includes('admin-config.js?v=20260612_OPS_ALERT_PREVIEW_SITE_1'), true, 'admin-studio.html should load the cache-busted admin config script for the visible rich-text yellow normalization fix');
...
test('shop admin order workflows externalize runtime table-row and modal styling', () => {
    ...
    assert.equal(adminStudioSource.includes('admin-config.js?v=20260612_OPS_ALERT_PREVIEW_SITE_1'), true, 'admin-studio.html should load the cache-busted admin config script');
...
test('analytics runtime renderers externalize heatmap, cohort, flow, and panel visibility styles', () => {
    ...
    const htmlMarkers = [
        ...
        'admin-config.js?v=20260612_OPS_ALERT_PREVIEW_SITE_1',
        'js/admin-payments.js?v=20260421_ADMIN_PAYMENTS_CONTEXT_HELPER_P2',
        'js/admin-workbench.js?v=20260612_OPS_ALERT_PREVIEW_SITE_2',
...
test('analytics user drill-down carries commerce context into the user detail modal', () => {
    ...
    assert.equal(adminStudioHtml.includes('admin-config.js?v=20260612_OPS_ALERT_PREVIEW_SITE_1'), true, 'admin-studio.html should reference the latest admin settings runtime version');
