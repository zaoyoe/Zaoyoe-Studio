...
test('homepage ships a static first-paint hero while runtime data hydrates', () => {
    ...
    assert.ok(
        indexSource.indexOf('./css/home-prompts-skeleton-critical.css?v=20260530_HOME_PROMPTS_SKELETON_GRID_1') < indexSource.indexOf('./css/framer_home.css?v=20260609_SUPPORT_COPY_MANUAL_1'),
        'prompt skeleton critical CSS should load before the deferred full homepage stylesheet'
    );
    assert.match(
        indexSource,
        /<link rel="stylesheet" href="\.\/css\/framer_home\.css\?v=20260609_SUPPORT_COPY_MANUAL_1&uiTextSelectGuard=20260530_UI_TEXT_SELECT_GUARD_1" media="print" data-deferred-style="1">/,
        'index.html should defer the full homepage stylesheet after the first-paint shell'
    );
    assert.equal(
        indexSource.includes('./css/framer_home.css?v=20260609_SUPPORT_COPY_MANUAL_1'),
        true,
