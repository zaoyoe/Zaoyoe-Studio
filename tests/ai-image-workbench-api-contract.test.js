...
test('ai image chat thread includes a Codex-style quick navigation rail', () => {
    ...
    assert.match(source, /const WORKBENCH_NARROW_QUERY = '\(max-width: 1120px\)'/);
    assert.match(source, /const CHAT_NAVIGATION_MIN_ITEMS = 2/);
    ...
    assert.match(source, /const mobileOffset = isMobileWorkbenchViewport\(\) \? 88 : 12/);
...
test('ai image workbench renders a scrollable history locator above the record list', () => {
    ...
    assert.match(source, /function renderHistoryLocator\(historyRows = \[\]\)/);
    assert.match(source, /data-aiw-history-locator/);
    assert.match(source, /data-aiw-history-locator-track/);
    assert.match(source, /data-aiw-history-nav-id="\$\{escapeHtml\(row\.id\)\}"/);
    assert.match(source, /data-aiw-history-nav-title/);
    assert.match(source, /data-aiw-history-locator-preview/);
    assert.match(source, /data-aiw-history-locator-preview role="tooltip" aria-hidden="true"/);
    assert.match(historyPreviewSource, /preview\.setAttribute\('aria-hidden', 'false'\)/);
    assert.match(historyPreviewSource, /preview\.setAttribute\('aria-hidden', 'true'\)/);
    assert.doesNotMatch(historyPreviewSource, /preview\.hidden = false/);
    assert.doesNotMatch(historyPreviewSource, /preview\.hidden = true/);
    assert.match(source, /function scrollToHistoryRow\(taskId = ''\)/);
    assert.match(source, /function showHistoryLocatorPreview\(button\)/);
    assert.match(source, /function hideHistoryLocatorPreview\(\)/);
    assert.match(source, /\$\{renderHistoryLocator\(filteredRows\)\}[\s\S]*<div class="ai-image-history">/);
    assert.match(cssSource, /\.ai-image-history-locator/);
    assert.match(cssSource, /\.ai-image-history-locator-track\s*\{[\s\S]*overflow-x: auto;[\s\S]*overscroll-behavior-x: contain;/);
    assert.match(cssSource, /\.ai-image-history-locator-preview\s*\{[\s\S]*max-height: 0;[\s\S]*opacity: 0;[\s\S]*transition:[\s\S]*max-height 0\.22s ease/);
    assert.match(cssSource, /\.ai-image-history-locator-preview\.is-visible\s*\{[\s\S]*max-height: 74px;[\s\S]*opacity: 1;/);
    assert.doesNotMatch(cssSource, /\.ai-image-history-locator-preview\[hidden\]\s*\{[\s\S]*display: none;/);
    assert.match(cssSource, /\.ai-image-task-row\.is-located/);
});
...
test('ai image workbench exposes video generation only from configured video models', () => {
    ...
    assert.match(source, /const maxHeight = getCssPixelValue\(styles\?\.maxHeight, 132\);/);
    assert.match(source, /const hasExplicitLineBreak = \/\\n\/\.test\(String\(textarea\.value \|\| ''\)\);/);
    assert.match(source, /const shouldGrow = hasExplicitLineBreak \|\| scrollHeight > minHeight \+ Math\.max\(4, Math\.round\(lineHeight \* 0\.45\)\);/);
    assert.match(source, /if \(!shouldGrow\) \{[\s\S]*textarea\.style\.height = '';[\s\S]*textarea\.scrollTop = 0;[\s\S]*return;[\s\S]*\}/);
...
test('ai image workbench uses a stable full-screen mobile layout', () => {
    const cssSource = fs.readFileSync(path.resolve(__dirname, '../css/ai-image-workbench.css'), 'utf8');
    assert.match(cssSource, /@media \(max-width: 1120px\) \{[\s\S]*\.ai-image-overlay\s*\{[\s\S]*min-height: 100dvh;/);
    ...
    assert.match(cssSource, /\.ai-image-main-prompt\s*\{[\s\S]*min-height: 58px;[\s\S]*color: var\(--aiw-muted\);[\s\S]*font: 650 14px\/1\.55 var\(--aiw-font\) !important;[\s\S]*-webkit-text-size-adjust: 100%;[\s\S]*text-size-adjust: 100%;/);
    assert.match(cssSource, /\.ai-image-main-prompt::placeholder\s*\{[\s\S]*color: color-mix\(in srgb, var\(--aiw-muted\) 56%, transparent\);[\s\S]*font: inherit;/);
    ...
    assert.match(cssSource, /@media \(max-width: 1120px\) \{[\s\S]*\.ai-image-dock\s*\{[\s\S]*bottom: max\(54px, env\(safe-area-inset-bottom, 0px\) \+ 42px\);/);
    ...
    assert.match(cssSource, /@media \(max-width: 1120px\) \{[\s\S]*\.ai-image-result-grid,[\s\S]*\.ai-image-result-grid--single\.ai-image-result-grid--wide\s*\{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/);
    assert.match(cssSource, /@media \(max-width: 1120px\) \{[\s\S]*\.ai-image-result-grid--chat\s*\{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/);
    assert.match(cssSource, /@media \(max-width: 1120px\) \{[\s\S]*\.ai-image-result-grid--video,[\s\S]*\.ai-image-result-grid--video\.ai-image-result-grid--thread\s*\{[\s\S]*grid-template-columns: minmax\(0, min\(360px, 100%\)\);/);
    assert.match(cssSource, /@media \(max-width: 520px\) \{[\s\S]*\.ai-image-result-grid--chat\s*\{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/);
    ...
    assert.match(cssSource, /@media \(max-width: 520px\) \{[\s\S]*\.ai-image-history-sidebar,[\s\S]*\.ai-image-history-sidebar\.is-expanded\s*\{[\s\S]*height: 60px;[\s\S]*max-height: 60px;/);
    ...
    assert.match(cssSource, /@media \(max-width: 520px\) \{[\s\S]*\.ai-image-history-expanded\s*\{[\s\S]*max-height: calc\(var\(--aiw-mobile-history-panel-max-height, min\(72dvh, 520px\)\) - 60px\);/);
    assert.match(cssSource, /@media \(max-width: 520px\) \{[\s\S]*\.ai-image-history-sidebar\.is-collapsed:hover \.ai-image-rail-btn,[\s\S]*\.ai-image-history-sidebar\.is-expanded \.ai-image-rail-btn\s*\{[\s\S]*width: 42px;[\s\S]*height: 42px;[\s\S]*justify-content: center;[\s\S]*gap: 0;[\s\S]*padding: 0;/);
    ...
    assert.match(cssSource, /@media \(max-width: 520px\) \{[\s\S]*\.ai-image-canvas\s*\{[\s\S]*padding-top: 60px;/);
...
test('ai image history supports multi-select delete pin and accent controls', () => {
    ...
    assert.match(source, /data-aiw-action="toggle-sidebar"/);
    assert.match(source, /ai-image-sidebar-toggle-chevron/);
    assert.match(source, /fa-chevron-left/);
    assert.match(source, /fa-chevron-up/);
    assert.doesNotMatch(source, /sidebarView \? 'fa-chevron-up' : 'fa-chevron-down'/);
    assert.match(source, /data-aiw-history-select/);
    assert.match(source, /data-aiw-history-selection-summary/);
    ...
    assert.match(source, /function syncHistorySelectionUi\(\)/);
    assert.match(source, /row\.classList\.toggle\('is-selected', selected\)/);
    assert.match(source, /toggleHistorySelection\(taskId\);\s*syncHistorySelectionUi\(\);/);
    ...
    assert.doesNotMatch(source, /toggleHistorySelection\(taskId\);\s*renderPreservingHistoryScroll\(renderHistoryPanelOnly\);/);
