...
test('shop purchase modal remains scrollable when the mobile keyboard docks it', () => {
    ...
    assert.match(
        shopClientSource,
        /bindContainedWheelIsolation: function \(scrollCard, \{ chainScrollTarget = null \} = \{\}\) \{[\s\S]*const getScrollEdgeBuffer = \(\) => \{[\s\S]*return Math\.min\(32, Math\.max\(0, paddingBottom\)\);[\s\S]*const effectiveMaxScrollTop = Math\.max\(0, maxScrollTop - getScrollEdgeBuffer\(\)\);[\s\S]*updatePurchaseGuidanceScrollState: function \(scrollCard\) \{[\s\S]*scrollCard\.classList\.toggle\('is-scroll-at-bottom', isAtBottom\);[\s\S]*scrollCard\.classList\.toggle\('is-scroll-away-from-bottom', isScrollable && !isAtBottom\);[\s\S]*bindPurchaseGuidanceScrollState: function \(scrollCard\) \{[\s\S]*this\.updatePurchaseGuidanceScrollState\(scrollCard\);[\s\S]*ensurePurchaseGuidanceVisible: function \(scrollCard\) \{[\s\S]*const purchaseScroll = document\.querySelector\('#shopPurchaseModal \.shop-purchase-scroll'\);[\s\S]*const visibleGap = Math\.max\(12, Number\.parseFloat\(rawGap\) \|\| 16\);[\s\S]*purchaseScroll\.scrollTop = Math\.min\(maxScrollTop, purchaseScroll\.scrollTop \+ overflowBottom\);[\s\S]*getPurchaseGuidanceWheelChainTarget: function \(\) \{[\s\S]*if \(this\.isWidePurchaseModalLayout\(\)\) \{[\s\S]*return null;[\s\S]*return document\.querySelector\('#shopPurchaseModal \.shop-purchase-scroll'\);[\s\S]*bindPurchaseNotesWheelIsolation: function \(\) \{[\s\S]*chainScrollTarget: this\.getPurchaseGuidanceWheelChainTarget\(\)[\s\S]*const scrollStateCleanup = this\.bindPurchaseGuidanceScrollState\(notesCard\);[\s\S]*bindPurchaseUsageWheelIsolation: function \(\) \{[\s\S]*chainScrollTarget: this\.getPurchaseGuidanceWheelChainTarget\(\)[\s\S]*const scrollStateCleanup = this\.bindPurchaseGuidanceScrollState\(usageCard\);[\s\S]*this\.ensurePurchaseGuidanceVisible\(notesCard\);[\s\S]*this\.ensurePurchaseGuidanceVisible\(usageCard\);/,
