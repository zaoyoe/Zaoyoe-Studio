...
test('requireAdmin accepts a valid admin studio cookie when bearer auth is unavailable', async () => {
    await withEnv({
        ...
        NEXT_PUBLIC_SUPABASE_ANON_KEY: undefined,
        SUPABASE_SERVICE_ROLE_KEY: 'service-key',
        ADMIN_STUDIO_ACCESS_SECRET: 'admin-studio-cookie-secret'
    }, async () => {
        const helpers = await loadAdminStudioAccessHelpers();
        const token = await helpers.issueAdminStudioToken({
            sub: 'admin-cookie-user',
        ...
        await withAdminModule(({ key }) => {
            if (key === 'service-key') {
                return {
                    ...
                    auth: {
                        ...
                        admin: {
                            async getUserById(userId) {
                                assert.equal(userId, 'admin-cookie-user');
                                return {
                                    data: {
                                        user: {
                    ...
                    async rpc(name, args) {
                        assert.equal(name, 'get_user_permissions');
                        assert.deepEqual(args, { p_user_id: 'admin-cookie-user' });
                        return {
                            data: {
                                is_admin: true,
                                is_super_admin: false,
            ...
            const result = await adminModule.requireAdmin({
                headers: {
                    cookie: `${helpers.getAdminStudioCookieName()}=${encodeURIComponent(token)}`
                }
            });

            assert.equal(result.user.id, 'admin-cookie-user');
            assert.equal(result.supabase.kind, 'admin');
            assert.equal(result.adminSupabase.kind, 'admin');
            assert.equal(result.requestSupabase, null);
            assert.equal(result.authSource, 'admin_studio_cookie');
        });
    });
});
