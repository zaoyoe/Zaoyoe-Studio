...
test('password reset invalid email feedback follows the active locale', () => {
    ...
    assert.match(injectAuthSource, /refreshVisibleAuthMessageTranslation\(\);/);
...
test('google popup callback is handed to the lightweight auth callback before the home page renders', () => {
    ...
    assert.match(authSource, /const GOOGLE_AUTH_DEBUG_STORAGE_KEY = 'zaoyoe_google_auth_debug_v1'/);
    ...
    assert.match(authSource, /function syncGoogleAuthDebugFlag\(\)/);
    assert.match(authSource, /function updateGoogleAuthDebugState\(label = '', detail = ''\)/);
    assert.match(authSource, /function formatGoogleAuthDebugSource\(source = ''\)/);
    ...
    assert.match(authSource, /function renderInlineGoogleButtons\(\)/);
    assert.match(authSource, /updateGoogleAuthDebugState\('当前入口：GIS 按钮'/);
    assert.match(authSource, /updateGoogleAuthDebugState\('当前入口：Popup Fallback'/);
    assert.match(authSource, /window\.google\.accounts\.id\.renderButton\(slot, \{/);
    assert.match(authSource, /use_fedcm_for_button: false/);
    ...
    assert.match(authSource, /source: 'popup_bridge'/);
    assert.match(authSource, /window\.handleGoogleCredentialResponse\(response, \{\s*source: 'gis_button'/);
