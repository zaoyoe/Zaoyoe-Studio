...
test('createPaymentRequest launches Hupijiao checkout and precreates the payment order with the real trade id', async () => {
    const state = {
        checkoutSessions: [],
        paymentOrders: [],
        pointsPackages: [
    ...
    global.fetch = async (url, request) => {
        assert.equal(String(url), 'https://api.xunhupay.com/payment/do.html');
        assert.match(String(request.body || ''), /trade_order_id=HJ_[A-Z0-9]{26}/);

        const responsePayload = {
            open_order_id: 'OPEN_HJ_1',
            url: 'https://pay.example.com/hupijiao/pay?id=1',
            url_qrcode: 'https://pay.example.com/hupijiao/qrcode?id=1',
            errcode: 0,
            errmsg: 'success!'
        };
    ...
    try {
        ...
        assert.equal(result.provider, 'hupijiao');
        assert.equal(result.mode, 'redirect');
        assert.equal(result.checkout_url, 'https://pay.example.com/hupijiao/pay?id=1');
        ...
        assert.match(state.paymentOrders[0].provider_order_no, /^HJ_[A-Z0-9]{26}$/);
        ...
        assert.equal(
            state.checkoutSessions[0].provider_metadata.gateway_open_order_id,
            'OPEN_HJ_1'
        );
    } finally {
