...
const SUPPORTED_ACTIONS = new Set(['context', 'messages', 'send_message']);
...
function normalizeRequestBody(body) {
    ...
    const page = normalizePage(body.page, { required: action !== 'messages' });
    ...
    if (action === 'send_message') {
        ...
        clientMessageId = normalizeBoundedString(body.client_message_id, 'client_message_id', 192, { required: true });
        if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,191}$/.test(clientMessageId)) {
...
async function createMessage(supabase, conversation, request) {
    ...
    if (isUniqueViolation(error)) {
        ...
        if (duplicate) return serializeMessage(duplicate);
