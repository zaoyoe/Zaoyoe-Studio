...
cd "$KVM4_ROOT"
if ! docker compose up -d --build --force-recreate verify-server ai-image-worker prompt-import-worker; then
  if [[ -n "$previous_app" && -d "$previous_app" ]]; then
    ...
    docker compose up -d --build --force-recreate verify-server ai-image-worker prompt-import-worker || true
  fi
  exit 1
fi
...
docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | grep -E 'NAMES|zaoyoe-verify-server|zaoyoe-ai-image-worker|zaoyoe-prompt-import-worker'
